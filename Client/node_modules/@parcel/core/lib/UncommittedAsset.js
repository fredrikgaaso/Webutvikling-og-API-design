...
var _serializer = require("@parcel/core/lib/serializer");
...
class UncommittedAsset {
  ...
  async commit(pipelineKey) {
    ...
    this.value.outputHash = (0, _rust().hashString)(((_this$value$hash = this.value.hash) !== null && _this$value$hash !== void 0 ? _this$value$hash : '') + pipelineKey + (await (0, _assetUtils.getInvalidationHash)(this.getInvalidations(), this.options)));
  ...
  getCacheKey(key) {
    return (0, _rust().hashString)(_constants.PARCEL_VERSION + key + this.value.id + (this.value.hash || ''));
  }
  addDependency(opts) {
